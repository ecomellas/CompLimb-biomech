    ...
    void Geometry::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry");
      {
        prm.declare_entry("Geometry type", "Ehlers_tube_step_load",
                           Patterns::Selection("Ehlers_tube_step_load|Ehlers_tube_increase_load"
                                               ...
                                               "|brain_growth_confined_drained"
                                               "|brain_growth_confined_undrained"
                                               "|brain_growth_unconfined_drained"
                                               "|brain_growth_unconfined_undrained"
        ...
        prm.declare_entry("Number of cycle sets", "1",
                          ...
                          "Budday_cube_tension_compression and "
                          "Budday_cube_tension_compression_fully_fixed. "
    ...
    void Materials::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material properties");
      {
        ...
        if ( (fluid_type == "Markert") && ((init_intrinsic_perm == 0.0) ||
                                           (viscosity_FR == 0.0))           )
            throw std::runtime_error ("Markert seepage velocity formulation "
        ...
        if ( (fluid_type == "Ehlers") && ((init_darcy_coef == 0.0) ||
                                          (weight_FR == 0.0))        )
            throw std::runtime_error ("Ehler seepage velocity formulation "
         ...
         epsilon_e_1 = epsilon_e_1_tr;
          ...
          while(residual_check > tolerance)
          {
            ...
            for (unsigned int a=0; a<dim; ++a)
            {
              ...
              iteration += 1;
              if (iteration>15)
                throw std::runtime_error ("No convergence in local Newton iteration "
                                ...
                                const Tensor<1,dim>      &N) const
          {
            if (this->parameters.load_type == "pressure")
            {
              if (boundary_id == 2)
              {
                ...
                const double load = initial_load + (final_load-initial_load)
                        *(current_time-initial_time)/(final_time-initial_time);
                return load * N;
              }
            }
          ...
          if (this->parameters.load_type == "pressure")
          {
            if (boundary_id == 2)
            {
              ...
              const double load = final_load * std::exp(r*current_time)
                                  / ( std::exp(c*current_time)
                                      + std::exp(r*current_time) );
              return load * N;
            }
          }
        ...
        make_dirichlet_constraints(ConstraintMatrix &constraints)
        {
          ...
          if (this->parameters.load_type == "displacement")
          {
              VectorTools::interpolate_boundary_values
                  (this->dof_handler_ref,
                   5,
                   ConstantFunction<dim>(this->get_dirichlet_load(5).first,
                                         this->n_components),
                   constraints,
                              ...
                              const Tensor<1,dim>      &N) const
        {
            if (this->parameters.load_type == "pressure")
            {
              if (boundary_id ==  5)
              {
                  ...
                  return ( final_load/2.0 *(1.0 - std::sin(numbers::PI*
                         (2.0*num_cycles*current_time/final_time + 0.5))) * N );
              }
            }
            ...
            if (boundary_id == 5)
            {
                ...
                if (this->parameters.num_cycle_sets == 1)
                {
                  ...
                  previous_displ = final_displ/2.0 * (1.0 - std::sin(numbers::PI
                    * (2.0*num_cycles*(current_time-delta_time)/final_time + 0.5)));
                }
                else
                {
                  if ( current_time <= (final_time*1.0/3.0) )
                  {
                      ...
                      previous_displ = final_displ/2.0 * (1.0 - std::sin(numbers::PI *
                          (2.0*num_cycles*(current_time-delta_time)/(final_time*1.0/3.0)
                           + 0.5)));
                  }
                  else
                  {
      ...
      private:
        ...
        make_dirichlet_constraints(ConstraintMatrix &constraints)
        {
          ...
          if (this->parameters.load_type == "displacement")
          {
              VectorTools::interpolate_boundary_values
                  (this->dof_handler_ref,
                   4,
                   ConstantFunction<dim>(this->get_dirichlet_load(4).first,
                                         this->n_components),
                   constraints,
                              ...
                              const Tensor<1,dim>      &N) const
        {
            if (this->parameters.load_type == "pressure")
            {
              if (boundary_id ==  4)
              {
                  ...
                  static const Tensor<2,dim,double>
                  R(Physics::Transformations::Rotations::rotation_matrix_3d(axis,angle));

                  return (final_load * (std::sin(2.0*(numbers::PI)*
                            num_cycles*current_time/final_time)) * (R * N));
              }
            }
        ...
        get_dirichlet_load(const types::boundary_id &boundary_id) const
        {
            ...
            if (boundary_id == 4)
            {
                ...
                if (this->parameters.num_cycle_sets == 1)
                {
                    current_displ  = final_displ * (std::sin(2.0*(numbers::PI)*
                        num_cycles*current_time/final_time));
                    previous_displ = final_displ * (std::sin(2.0*(numbers::PI)*
                        num_cycles*(current_time-delta_time)/final_time));
                }
                else
                {
                    AssertThrow(false,
                      ExcMessage("Problem type not defined. "
                      "Budday shear experiments implemented only for one set of cycles."));
                }
                displ_incr = current_displ - previous_displ;
